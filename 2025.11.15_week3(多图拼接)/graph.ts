...
export class ChatbotGraph {
  ...
  private buildGraph() {
    const builder = new StateGraph(StateAnnotation)
      ...
      .addNode("planningAgent", this.planningAgent.bind(this))
      // 工具节点
      ...
      .addNode("cameraStitch", this.cameraStitch.bind(this))  //新增拼图
      ...
      .addEdge("cameraStitch", "aggregateResults")  //新增拼图
    ...
    for (const toolCall of toolCalls) {
      switch (toolCall.toolName) {
        ...
        case "get_camera_snapshot":
          ...
          break;
        // 新增拼图
        case "stitch_camera_images":
          routes.add("cameraStitch");
          break;
        default:
...
When controlling devices, always use the available tools. The tools will handle the actual device control.`;
  ...
  //新增拼图
  private async cameraStitch(state: GraphState) {
    return this.executeToolCalls(state, 'stitch_camera_images');
  }

  /**
   * Generic tool execution helper
   */
